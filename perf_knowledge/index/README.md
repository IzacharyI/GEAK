...
- **See how somebody else answered a design question** → [`../corpus/gemm_family.md`](../corpus/gemm_family.md),
  the same operator family written six ways, indexed per decision with `file:line`. The complement to
  `run_recurrence.md`: that one aggregates *what paid when measured*, this one records *what was
  chosen*, read straight off source at a named commit. Neither ranks anything.
