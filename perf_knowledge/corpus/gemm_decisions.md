...
- Put the preshuffled B path on the candidate list on the generic family instead of deferring it: b_preshuffle is the shipped default of the public entry point (flydsl_hgemm) and of the internal compile helper, so the unshuffled path you start from is the deviation from the library's own choice, not the safe baseline.
