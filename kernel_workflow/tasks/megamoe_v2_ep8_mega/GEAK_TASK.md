# GEAK task (MEGA mode): MegaMoE V2, EP8 intra-node — author the complete fused megakernel in one design
...
## DIRECTIVE — whole-fusion edit lane (authoritative; overrides any inferred file set)
...
2. **Liveness**: the operator is captured into a CUDA Graph and replayed. A fused kernel with
   cross-rank waits can deadlock on replay N without failing on replay 1. **Replay COUNT is not the
   coverage knob — replay DIVERSITY is.** Identical replays re-run one interleaving and can
   ...
   - **Any candidate with a per-token / cross-rank arrival race** (the optional per-token readiness
     ...
     routes, under a bounded per-replay timeout.

   A jittered 256 covers the replay-N race class strictly better than an identical 1000. A timeout is
   a FAILURE, never a skip. The terminal credit read for `8192_uniform` uses the ≥256 jittered gate.
